...
1. In the term "dimensionality reduction", "dimension" refers to the columns of a tidy dataframe; also known as variables or features. However, we are not simply removing columns; we are trying to come up with a set of new columns, fewer in number than the original, which best continuously quantify in what ways rows are similar and different to each other.

For example, in a dataset of students in a gym class, we might have columns for many different physical wellness tests; how fast they can run a mile, their score on the PACER aerobic capacity test, the number of pushups they can do, and the number of squats they can do.

Using all four columns gives us the most accurate, full picture of the students' abilities. However, dimensionality reduction poses the question - how would we best describe the students using fewer than four columns of information?

Simply removing a set of the existing columns gets rid of valuable information, so instead we seek we combine the columns in a meaningful way. For example, to describe these students with two columns of information, we might come up with a continuous numeric "endurance" score that captures their mile time and PACER score, and a "strength" score that captures their pushups and squats.

These scores, by definition of being a reduction, are not as fruitful and informative as the original columns. However, when dimensionality reduction is done well on an appropriate dataset, it can be extremely useful, especially when you don't start with four columns, but hundreds or thousands. This is the case in fields like genomics, astronomy, and many others.
