...
```{r}
ui <- fluidPage(
  selectInput("cityDropdown", 
              "Choose your city or cities", 
              multiple = TRUE, 
              selected = c("Bockel", "Dibbersen", "Ascheberg"), 
              choices = unique(traffic$name)),
  plotOutput("myTimePlot")
  
)

server <- function(input, output) {
  chosenCities = reactive({
    traffic %>% 
      filter(name %in% input$cityDropdown)
  })
  
  output$myTimePlot = renderPlot({
  ggplot(chosenCities(), aes(date, value, color = name)) +
    geom_line()
  })
}
