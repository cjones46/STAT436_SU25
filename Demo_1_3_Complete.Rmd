...
a. Construct three base `ggplot2` plots based on the variables that are provided. 
For example, you may construct a line plot (`geom_line`) of average precipitation, a histogram (`geom_histogram`) of per-city windspeeds, or a ribbon plot (`geom_ribbon`) of temperature ranges over time. Make sure to display at least two weather variables from among the four that are provided.

```{r}
line_plot = ggplot(weather, aes(day_in_year, precipitation, color = location)) +
  geom_smooth(se = F) +
  theme(
...
```{r}
histogram = ggplot(weather, aes(wind, fill = location)) +
  geom_histogram() +
  ggtitle("a")
...
```{r}
ribbon_plot = ggplot(weather, aes(day_in_year, ymin = temp_min, ymax = temp_max, fill = location)) +
  geom_ribbon() +
  ggtitle("c")
```
...
```{r}
histogram + (line_plot / ribbon_plot) +
