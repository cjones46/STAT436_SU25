...
leads to the other (such as one social media account following another, but not necessarily reverse), while in an undirected edge, direction doesn't even make sense to consider (e.g. two senators are connected if they are the same political party).
...
G <- tbl_graph(edges = E)
# By default, G assumes these edges have direction; this can be adjusted by tbl_graph(edges = E, directed = FALSE)
G
```

This `tbl_graph` can be plotted using the code below. 

```{r}
ggraph(G, layout = "kk") + 
  geom_edge_link() + # like geom_line
  geom_node_point() # like geom_point
```

The `layout = "kk"` argument is just one of many options that will change the arrangement of the points on your graph (but not the actual structure the network itself); see more information about layouts [here](https://ggraph.data-imaginist.com/articles/Layouts.html). Here's an alternative layout for the same network:

```{r}
ggraph(G, layout = "eigen") + 
  geom_edge_link() +
  geom_node_point()
```

There are different geoms available for nodes and edges -- for example, here's what happens if you replace `geom_edge_link()` with `geom_edge_arc()`.
...
This function might seem silly in this layout, but in the `linear` layout it makes a lot more sense:
...
6. We can mutate node and edge attributes using dplyr-like syntax. By default, you'll be editing the nodes of the graph; you can switch over to activating the edges with `activate(edges)`.

```{r}
G <- G %>%
  # Give each node an ID and a group
  mutate(
    id = row_number(),
    group = id < 4
  ) %>%
  # Switch over to the edges, give each one a width
  activate(edges) %>%
...
the `geom_edge_link` and `geom_node_label` geoms.
